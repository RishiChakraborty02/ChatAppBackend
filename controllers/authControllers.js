...
const login = async (req, res) => {
    try {
        ...
        res.status(200).json({ token, username: user.rows[0].username, user_id:user.rows[0].user_id});
    } catch (error) {
        console.log(error);
...
const register = async (req, res) => {
    try {
       const {username,email,password}=req.body
        ...
        res.status(200).json({ token, username: newUser.rows[0].username, user_id:user.rows[0].user_id });
    } catch (error) {
        console.log(error);
